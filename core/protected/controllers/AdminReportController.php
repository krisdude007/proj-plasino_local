...
//            'socialToTvCount' => eTicker::model()->filterByTickerDate($filterDate)->social()->with('tickerDestination')->filterByDestination($tvId)->count(),
    ...
    public function actionRegisteredUserReport() {
        ...
        $userReportData = Yii::app()->db->createCommand("SELECT u.id, u.username, u.first_name, u.last_name, u.created_on, up.number, ul.postal_code FROM user u LEFT OUTER JOIN user_phone up ON (up.user_id=u.id) AND (up.type = 'primary') LEFT OUTER JOIN user_location ul ON (ul.user_id=u.id) AND (ul.type = 'primary') WHERE (u.role = 'user' AND u.created_on != '0000-00-00 00:00:00' AND u.created_on >= '".$startDate."')")->queryAll();
        ...
        //$videoArray = eUser::model()->findAll($criteria);var_dump($videoArray);exit;
        $df = fopen("php://output", 'w');
        fputcsv($df, $headerArray);
        foreach($userReportData as $row) {
            $id = $row['id'];
            $number = isset($row['number']) ? $row['number'] : '';//var_dump($number);
...
//            else
//                $postal_code = '';
            $videosCount = eVideo::model()->count("user_id=:user_id", array("user_id" => $id));
            $row = array($id,
                //$val->id,
                //$val->username,
                //$val->first_name,
